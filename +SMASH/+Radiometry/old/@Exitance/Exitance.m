% This class creates Exitance objects for radiometry calculations.
% Exitance is radiation power emitted from a radiator surface.  
% The SI unit of exitance is watts per square meter [W.m^-2].
%
% The most direct way of creating a Exitance object is to pass 3 input
% arrays:
...
% (MxN) array.
%
% See also Radiation, Radiance, Flux, Irradiance, Intensity
%

% created March 27, 2014 by Tommy Ao (Sandia National Laboratories)
%
classdef Exitance < SMASH.Radiometry.Radiation
    %% properties
    ...
    %% constructor
    methods (Hidden=true)
        function object=Exitance(varargin)
            object=object@SMASH.Radiometry.Radiation(varargin{:});
            object.Name='Exitance object';
            object.Title='Exitance object';
            % handle input
            if nargin==0
            ...
            if grid1Length>1
                object.DataLabel='Spectral Exitance (W·m^{-2}·nm^{-1})';
            else
                object.DataLabel='Exitance (W·m^{-2})';
            end
            % handle time array

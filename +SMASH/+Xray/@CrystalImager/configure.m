...
%
% See also CrystalImager
%

%
% created March 8, 2017 by Patrick Knapp (Sandia National Laboratories)
%
...
if nargout==0
    ...
    format=sprintf('\t%%%ds : ',L);
    % print TIPC settings
    fprintf('*** Crystal Imager analysis settings ***\n');
    name=fieldnames(object.Settings);
    name=sort(name);
    for k=1:numel(name);
...
for n=1:2:Narg
    ...
    switch lower(name)
        ...
        case 'filters'
            ...
            assert(numel(value.(fnames{1})) == numel(value.(fnames{2})), ...

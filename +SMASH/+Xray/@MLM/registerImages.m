...
for i = 1:length(varargin)
    ...
    elseif strcmpi(varargin{i},'manual'); manual = true;
    end
end
...
shifts = zeros(2,Nchannels);
new_shifts = zeros(2,Nchannels);
xCrop = Parameters.FinalImageXLims;
yCrop = Parameters.FinalImageYLims;


if manual
    img1 = SMASH.ImageAnalysis.Image(object.Frames.Grid1,object.Frames.Grid2,object.Frames.Data(:,:,ref));
    
    if isempty(object.Settings.Shifts)
    ...
    images_reg = zeros(size(img1.Data,1),size(img1.Data,2),Nchannels);
    images_reg(:,:,ref) = img1.Data;
    
    for i = 1:Nchannels;
        if i ~= ref
            xs = object.Settings.Shifts(:,i);
            moving = SMASH.ImageAnalysis.Image(object.Frames.Grid1,object.Frames.Grid2,object.Frames.Data(:,:,i));
            moving = shift(moving,'Grid1',xs(1)); moving = shift(moving,'Grid2',xs(2));
            
            [registered, shift_i] = registerGUI(img1, moving);
            images_reg(:,:,i) = registered.Data;
            new_shifts(:,i) = shift_i';
        end
    end
    imgs_reg = SMASH.ImageAnalysis.ImageGroup(img1.Grid1,img1.Grid2,images_reg);
    object.RegisteredImages = imgs_reg;
    object.RegisteredImages.Legend = object.Frames.Legend;
    object.Settings.Shifts = object.Settings.Shifts + new_shifts;
    ...
    for i = 1:Nchannels
        if i ~= ref
            ...
            img2 = SMASH.ImageAnalysis.Image(object.Frames.Grid1,object.Frames.Grid2,object.Frames.Data(:,:,i));
            ...
            shifts(:,i) = [-(img2_interp.Grid1(J)-img1_interp.Grid1(1)); -(img2_interp.Grid2(I)-img1_interp.Grid2(1))];
    ...
    object.RegisteredImages = imgs_reg;
    object.RegisteredImages.Legend = object.Frames.Legend;
    object.Settings.Shifts = shifts;
end
object.RegisteredImages.GraphicOptions.AspectRatio = 'equal';
object.RegisteredImages.GraphicOptions.YDir = 'normal';

...
BoxSize = [2.0,2.0];    % segmentation image size in centimeters
...
object.Linearized.DataLim = DataLim;
h = view(object.Linearized);
hold all
...
for n = 1:length(Xc)
    ...
    images{n} = crop(object.Linearized,[Xc(n) - BoxSize(1)/2, Xc(n) + BoxSize(1)/2],[Yc(n) - BoxSize(2)/2, Yc(n) + BoxSize(2)/2]);
...
for i = 1:object.Settings.NumberImages
    object.Frames.Legend{i} = ['t',num2str(i)];
end

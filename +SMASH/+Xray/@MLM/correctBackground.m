...
%
% See also MLM, Xray, Image, Radiography
...
for i = 1:length(varargin)
    ...
    elseif strcmp(varargin{i},'CLims'); CLims = varargin{i+1};
    end
end

if isempty(object.Linearized)
    img = object.Measurement;
else
    img = object.Linearized;
end
img.GraphicOptions.AspectRatio = 'equal';
CLim_old = img.DataLim;
img.DataLim = CLims;
...
switch Method
    ...
    case 'Polynomial'
        ...
        img_bkg = subtractBackground(img,'poly_order',Parameters.PolyOrder,'ShowPlot',show);        
end

img_bkg.GraphicOptions.AspectRatio = 'equal';
img_bkg.GraphicOptions.YDir = 'normal';
object.Linearized = img_bkg;

object.Linearized.GraphicOptions.LineColor = 'm';
object.Linearized.GraphicOptions.LineStyle = '--';
object.Linearized.GraphicOptions.YDir = 'normal';
object.Linearized.Grid1Label = 'Radial Distance [cm]';
object.Linearized.Grid2Label = 'Axial Distance [cm]';
object.Linearized.DataLabel = 'Exposure';
object.Linearized.DataLim = CLim_old;

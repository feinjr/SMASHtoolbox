...
%    >> new=cloneAxes(haxes); % "haxes" is the source handle
% A figure handle:
%    >> new=cloneAxes(fig); % source axes inside figure "fig"
...
% present, it is cloned automatically.  Calling the function with now
% inputs:
%    >> new=cloneAxes;
% prompts the user to select from a list of visible figures.  This figure
% is the displayed (if more than one axes is present) for source selection.
%  
% MATLAB uses special axes for legends and colorbars.  This function does
...
%
function varargout=cloneAxes(target)

% handle input
if (nargin<1) || isempty(target)
    target=selectFigure;
    new=SMASH.Graphics.cloneAxes(target);

function ReturnWavelength=MatchWavelength(x,y,centerWavelength,grating)
    ...
    PeaksFile = fullfile('HgNe Lamp Data','Peak Pixels and Wavelengths.csv');
    DataFile=fullfile('HgNe Lamp Data','Visible HgNe Wavelengths.csv');
    LampData=csvread(DataFile,1);
    LampPeaks=csvread(PeaksFile,1,1);

    LampX=LampData(:,1);
    LampY=LampData(:,2);
    LampX=LampX*order;

    PeakPixel=LampPeaks(:,1)*order;
    PeakWavelength=LampPeaks(:,2);
    PixelCoeff=LampPeaks(1:4,3);
    ...
    b = uicontrol('Parent',fig,'Style','slider','Units','normalized','Position',[ax.Position(1) 0 ax.Position(3) .03],...
     ...
     function MovePlot(b,~)
        ...
        min(LampY(round(PeakPixel/order)) )

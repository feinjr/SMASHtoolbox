...
% to the 'Choose Active Signals' option in the edit menu bar. The next 
...
% Sesame : Add a Sesame format tabulation. eospath defines a hardcoded
%   location to search for the sesame files due to export control issues.
%   It is suggested the user change this path. If intersect pressure is 0 
%   the principal Hugoniot is generated. If it is nonzero, then a reshock 
%   or release from this pressure (assumed to be on principal Hugoniot) is
%   generated.
% Rayleight line : Construct a Rayleigh line from an initial density and
%   shock velocity. 
% Load file : Load a tabular (u,P) file
% Labels : change the default name of each curve
% Comment : curves are stored as signal objects. Comments can be added.
% Save : Save each selected curve (Signal object)

%% Analysis
% Calculate intersections : Find unique intersection points for all of the
%   selected curves

%% Plotting
% The same plotting features as SignalGUI
...
uimenu(hm,'Label','Impact Calculator','Callback',@ImpCalc);
...
%Generate curve based on shock data: c0, s
function ImpCalc(src,varargin)
    
% see if dialog already exists
dlg=FindOrCreateDlg(src,'ImpactCalculator');
if ishandle(dlg)
    return
end
...
%Build dialog
dlg.Hidden = true;
dlg.Name = 'ImpactCalculator';
target_h=addblock(dlg,'popup','Target',MGmats);
impactor_h=addblock(dlg,'popup','Impactor',MGmats);
iv_h=addblock(dlg,'edit','Impact Velocity (km/s)        '); set(iv_h(2),'String', 0);
p_h= addblock(dlg,'edit','Pressure (GPa)                '); set(p_h(2),'String', 0);
up_h=addblock(dlg,'edit','Particle Velocity (km/s)      '); set(up_h(2),'String', 0);
ts_h=addblock(dlg,'edit','Target Shock Velocity (km/s)  '); set(ts_h(2),'String', 0);
td_h=addblock(dlg,'edit','Target Shock Density (km/s)   '); set(td_h(2),'String', 0);
...
set(impactor_h,'Callback',@ApplyCallback);
set(iv_h,'Callback',@ApplyCallback);
set(p_h,'Callback',@ApplyCallback);
set(up_h,'Callback',@ApplyCallback);

function ApplyCallback(varargin)
    
    value = probe(dlg);
    
    nt = find(strcmpi(value{1},MGmats),1,'First');
    ni = find(strcmpi(value{2},MGmats),1,'First');
    
    dt = MGd(nt); ct = MGc(nt); st = MGs(nt);
    di = MGd(ni); ci = MGc(ni); si = MGs(ni);
    
    %Determine what triggered the callback
    whichh=get(varargin{1},'Position');
    hiv=get(iv_h(2),'Position');
    hp=get(p_h(2),'Position');
    hup=get(up_h(2),'Position');
    

    if all(whichh==hp)
        p = str2double(value{4});
        f=@(up) p-(dt.*(ct+st.*up).*up);
        up=fzero(f,1)
        f=@(iv) (dt.*(ct+st.*up).*up)-(di.*(ci+si.*(iv-up)).*(iv-up));
        iv=fzero(f,up*2)       
    elseif all(whichh==hup)
        up = str2double(value{5});
        p = (dt.*(ct+st.*up).*up);
        f=@(iv) (dt.*(ct+st.*up).*up)-(di.*(ci+si.*(iv-up)).*(iv-up));
        iv=fzero(f,up*2);
    else
        iv = str2double(value{3});
        f=@(up) (dt.*(ct+st.*up).*up)-(di.*(ci+si.*(iv-up)).*(iv-up));
        up=fzero(f,iv/2);
        p = (dt.*(ct+st.*up).*up);   
    end
    ...
    set(iv_h(2),'String',num2str(iv));
    set(p_h(2),'String',num2str(p));
    set(up_h(2),'String',num2str(up));
    set(ts_h(2),'String',num2str(ts));
    set(td_h(2),'String',num2str(td));
    set(is_h(2),'String',num2str(is));
    set(id_h(2),'String',num2str(id));

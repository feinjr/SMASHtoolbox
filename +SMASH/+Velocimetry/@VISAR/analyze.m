...
%
% This method analyzes the VISAR object to provide the Quadrature, 
% Fringe Shift, Contrast, and Velocity signals.  This involves filtering the
% signals and applying the vertical offsets and scalings.  The possilble 
% syntaxes are listed below.
%     >> object=analyze(object,filter,params)
%     >> object=analyze(object,filter)
...
%  'None'       - This is the default.  Any value entered for params is 
%                 ignored
...
function object=analyze(object,FilterType,params)
...
if (nargin<2) || isempty(FilterType)
    ...
    params=[];
elseif nargin == 2
    if ischar(FilterType) ~= 1
        error('ERROR: Invalid Filter Name');
    end
    params=[];
elseif nargin == 3
    assert(ischar(FilterType),'ERROR: Invalid Filter Name');
end
...
object.Velocity=object.InitialVelocity+object.VPF*object.FringeShift;

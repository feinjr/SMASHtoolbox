...
function history=analyzeSinusoid(measurement,boundary,setting,harmonic)

% manage input
measurement=SMASH.SignalAnalysis.ShortTime(measurement);

Nboundary=numel(boundary);

if (nargin<4) || isempty(harmonic)
    harmonic=[1 1];
end
assert(size(harmonic,2)==2,'ERROR: invalid harmonic table');
if size(harmonic,1)==1
    harmonic=repmat(harmonic,[Nboundary 1]);
end
assert(size(harmonic,1)==Nboundary,'ERROR: invalid harmonic table');

% setup functions
tleft=cell(1,Nboundary);
tright=cell(1,Nboundary);
    function parameter=processBlock(t,s)        
        parameter=nan(4,Nboundary); % [center amplitude chirp unique]        
        for m=1:Nboundary
            % find boundary selections in analysis domain
            tb=probe(boundary{m});
            keep=(tb>t(1)) & (tb<t(end));
            tb=tb(keep);
            tb=[t(1); tb(:); t(end)];                        
            [flo,fhi]=probe(boundary{index},tb);
            if any(isnan(flo)) || any(isnan(fhi))
                continue
            end
            fb=(fA+fB)/2;
            Dfb=(fB-fA)/2;                                                                    
            ...
            while true
                ...
                for n=1:Nm
                    ...
                    tR=tright{m}(n);
                    keep=(t>=tL) & (t<=tR);
                    p=polyfit(tb(keep),fb(keep),1);
                    temp=polyval(p,tb(keep));
                    temp=abs(temp-fb(keep));
                    err(keep)=max(err(keep),temp);
                end
                [value,index]=max(err);
                if value>setting.EffectiveWidth
                    if index==1
                        index=2;
                    elseif index==Nm
                        Nm=Nm-1;
                    end
                    % insert new break point into tleft and tright
                    continue
                else
                    break
                end
...
history=analyze(measurement,@processBlock);
return
